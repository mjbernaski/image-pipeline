...
def generate_prompt(steering_concept=None, image_base64=None, return_details=False, model=None, temperature=None):
    ...
    try:
        ...
        prompt = re.sub(r'<reasoning>.*', '', prompt, flags=re.DOTALL)
        prompt = re.sub(r'<\|begin_of_thought\|>.*?<\|end_of_thought\|>', '', prompt, flags=re.DOTALL)
        prompt = re.sub(r'<\|begin_of_thought\|>.*', '', prompt, flags=re.DOTALL)
